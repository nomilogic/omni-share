  ...
  useEffect(() => {
    ...
    const loadPreferences = async () => {
      const prefs = JSON.parse(
        localStorage.getItem("ai-model-preferences") || "{}"
      );
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center space-x-2">
          <div
            className={`w-8 h-8 rounded-md flex items-center justify-center ${getProviderColor(
              model.provider
            )} bg-opacity-20`}
          >
            {(() => {
              ...
              return (
                <IconComponent
                  className={`w-4 h-4 ${
                    getProviderColor(model.provider).split(" ")[1]
                  }`}
                />
              );
            })()}
      ...
      <div className="mt-2">{getPerformanceBadge(model.id)}</div>
    </div>
  );
